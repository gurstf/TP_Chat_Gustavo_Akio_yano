...
class CipheredGUI:
    ...
    def run_chat(self, sender, app_data)->None:
        ...
        size_crypto = 16
        iterations = 100000
        salt = os.urandom(size_crypto)

        kdf = PBKDF2HMAC(hashes.SHA256(), size_crypto, salt, iterations)
    ...
    def encrypt(text,self):
        ...
        size = algorithms.AES128.block_size
        padded = padding.PKCS7(size).padded()
        padder_message = padded.update(text.encode()) + padded.finalize()


        encrypted_text = encrypter.update(padder_message) + encrypter.finalize()
        return(iv,encrypted_text)

    def decrypt(text,self):
        message = base64.b64endoce(text[1]['data'])
        iv = base64.b64encode(text[0]['data'])
        cipher = Cipher(algorithms.AES(self._key), modes.CTR(iv), backend=default_backend()) 
        decrypter = cipher.decrypter()
        decrypted_text = decrypter.update(message) + decrypter.finalize()
        unpadder_message = padding.PKCS7(128).unpadder_message()
        unpadded = unpadder_message.update(decrypted_text) + unpadder_message.finalize()
    ...
    def recv(self)->None:
        ...
        if self._callback is not None:
            for user, message in self._callback.get():
                self.update_text_screen(f"{user} : {mself.decrypt(text[0]['data'], text[1]['data'])}")
